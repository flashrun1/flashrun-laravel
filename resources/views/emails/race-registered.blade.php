
<p style="font-size: 18pt;">
    Привіт <b>{{ $request->name }}</b>!<br>
    Дякуємо за реєстрацію на забіг <b>{{ $request->race_name }}</b>!<br>
    Для участі потрібно оплатити стартовий внесок, вартість 600грн<br><br>
    Реквізити нижче: <br>
    Отримувач: ГО ШВИДКИЙ БІГ <br>
    Код отримувача: 44225232 <br>
    Рахунок: UA033154050000026009052337925 <br>
    Назва банку: ХМЕЛЬНИЦЬКА ФIЛIЯ АТ КБ "ПРИВАТБАНК" <br> <br>
    Після оплати будь ласка напишіть на <a href="mailto:rybak.snizhana@gmail.com">пошту rybak.snizhana@gmail.com</a> або в <a href="https://www.instagram.com/flashrun_khm/">instagram @flashrun_khm</a>
    <br>
</p>
<p style="font-size: 18pt;">
    QR-код для зручної оплати у Приват24 <br>
    <img src="{{ asset('/images/qr.png') }}" height="200" width="200" alt="">
</p>

Дякуємо, Слава Україні!
<hr>

Flashrun

